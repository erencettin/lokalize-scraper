"""Entry point for Biletinial affiliate feed sync pipeline."""
...
def main() -> int:
    _logger.info("=== biletinial sync started ===")

    events: List[NormalizedEvent] = []
    try:
        events = BiletinialProvider().fetch_and_parse()
        _logger.info("biletinial: %s events", len(events))
    except Exception as exc:
        _logger.error("BiletinialProvider failed: %s", exc)
        return 1
    ...
    sync_run_id = f"biletinial-{uuid.uuid4().hex[:8]}-{datetime.now().strftime('%H%M%S')}"
    ...
    try:
        sync_service.trigger_stale_cleanup(sync_run_id, provider="Biletinial")
    except Exception as exc:
        _logger.error("Stale cleanup failed: %s", exc)

    _logger.info("=== biletinial sync finished ===")
    return 0
