"""Custom parser for Bağcılar Belediyesi."""
...
class BagcilarParser(SiteParser):
    """Parse Bağcılar list and detail pages."""

    def parse_list(self, html: str, site: MunicipalSite) -> List[RawEventItem]:
        links = set()
        patterns = (
            r'href="(?P<link>https?://(?:www\.)?bagcilar\.bel\.tr/(?:etkinlikler/[^"#?]+|Activity/Detail/\d+))"',
            r'href="(?P<link>/(?:etkinlikler/[^"#?]+|Activity/Detail/\d+))"',
        )
    ...
    def parse_detail(self, html: str, item: RawEventItem, site: MunicipalSite) -> RawEventItem:
        ...
        item.venue = item.venue or extract_label_value(html, ["Etkinlik Yeri", "Yer", "Mekan"]) or site.name
        item.date = item.date or extract_label_value(html, ["Etkinlik Tarihi", "Tarih", "Seans Tarihi"])
        item.time = item.time or extract_label_value(html, ["Etkinlik Saati", "Saat", "Seans"])
        if not item.date:
